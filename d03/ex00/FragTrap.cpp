...
void FragTrap::attackOne( std::string const &target )
{
	std::cout << "Defragmenting " << target << std::endl;
}

void FragTrap::attackTwo( std::string const &target )
{
	std::cout << "Running the sequencer on " << target << std::endl;
}

void FragTrap::attackThree( std::string const &target )
{
	std::cout << "Let's get this party started with " << target << std::endl;
}

void FragTrap::attackFour( std::string const &target )
{
	std::cout << "Resequencing combat protocols of " << target << std::endl;
}

void FragTrap::attackFive( std::string const &target )
{
	std::cout << "I'm pulling tricks outta my hat for " << target << std::endl;
}
...
void FragTrap::vaulthunter_dot_exe(std::string const &target)
{
	if (this->_EnergyPoints >= 25)
    {
        ...
        Attack attack[5] = {&FragTrap::attackOne, &FragTrap::attackTwo, &FragTrap::attackThree,
                              &FragTrap::attackFour, &FragTrap::attackFive};
        int rand = std::rand() % 5;
        (this->*(attack[rand]))(target);
